...
bot.onText(/\/tip(?:\s+(@\S+))?\s+(\d+(\.\d+)?)/, async (msg, match) => {
  ...
  try {
    ...
    if (msg.reply_to_message) {
      ...
      // Get username from the original message or fallback to stored username
      recipientDisplayName = msg.reply_to_message.from.username || recipient.username || `User${recipientTelegramId.slice(-4)}`;
    ...
    const dmMsg = await bot.sendMessage(
      tipperTelegramId,
      `You are tipping ${tipAmount} STX to @${recipientDisplayName}.\n` +
    ...
    const replyListener = async (replyMsg) => {
      try {
        ...
        // Get tipper display name
        const tipperDisplayName = msg.from.username || `User${tipperTelegramId.slice(-4)}`;

        // For DM-initiated commands, send the TX link to both tipper and recipient.
        if (msg.chat.type === 'private') {
          await bot.sendMessage(
            tipperTelegramId,
            `✅ ${tipAmount} STX reward sent successfully to @${recipientDisplayName}! 🎓\nTX ID: ${explorerLink}\n\n💡 Great job rewarding academic excellence!`
          );
          if (recipient.telegramId) {
            await bot.sendMessage(
              recipient.telegramId,
              `🎉 Congratulations! You've received an academic reward of ${tipAmount} STX from @${tipperDisplayName}! 🎓\n\nKeep up the excellent work!\nTX ID: ${explorerLink}`
            );
          }
        } else {
          ...
          await bot.sendMessage(
            chatId,
            `🎓 Academic Reward: ${tipAmount} STX from @${tipperDisplayName} to @${recipientDisplayName}!\n\n✨ Recognizing academic excellence and positive behavior!`,
            { reply_to_message_id: msg.message_id }
          );
        }
      } catch (error) {
